    ...
    for (long i = (*nLoc); i < sCurInput.length(); i++)
    {
		...
		if (ch == '<')
		{
			...
			else if ((nMode == PARSE_MODE_ELEMENT_DATA) || (nMode == PARSE_MODE_CDATA_END))
			{
				...
				//V0.94 - check for CDATA elements that are like:
				// "<blah><![CDATA[my cdata]]></blah>"
				if (sCurInput.substr(i+1, 7).compare("![CDATA") == 0)
				{
					nMode = PARSE_MODE_ELEMENT_PROVISIONAL;
				}
				else
		...
		else if (ch == ']')
		{
		    ...
		    if (nMode == PARSE_MODE_CDATA)
			{
				...
				//V0.94 - check for "<blah><![CDATA[my cdata]]></blah>"
				if (sCurInput.substr(i+1, 1).compare(">") == 0) i++;
			}
...
int FXMLParser::ReadFile(const char *cFilename, std::string &Str)
{
  ...
  std::ifstream File(cFilename, ios::in);
  //open the file
  if(File)
  {
